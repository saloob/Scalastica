...
# case 'AccountsServices':
  ...
  */
  ...
  if ($valtype == 'AccountsServices' && $action == 'add'){
     #$sclm_accountsservices_id_c = $val;
     $ci_params[0] = " id='".$val."'" ;
     }
  ...
  switch ($action){
   ...
   case 'search':
    ...
    if ($valtype == 'AccountsServices'){
       echo "<div style=\"".$custom_portal_divstyle."\"><center><font size=3 color=".$portal_font_colour."><B>".$strings["BaseServices"]."</B></font></center></div>";
       } else {
       echo "<div style=\"".$custom_portal_divstyle."\"><center><font size=3 color=".$portal_font_colour."><B>".$strings["CatalogServices"]."</B></font></center></div>";
       }
...
?>
 <div style="<?PHP echo $divstyle_white; ?>">
  <center>
...
//              $tier = "[<a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=Services&action=list&value=".$service_tier."&valuetype=ConfigurationItems');return false\">".$service_tier_name."</a>] ";
              ...
              $cis .= "<div style=\"".$divstyle_white."\"><div style=\"margin-left:0px;float:left;width:98%;height:60px;border-radius:0px;padding-left:0px;padding-top:0px;padding-right:0px;padding-bottom:0px;\"><div style=\"margin-left:0px;float:left;width:55px;height:55px;border-radius:0px;padding-left:0px;padding-top:0px;padding-right:0px;padding-bottom:0px;\"><center><a href=\"#\" onClick=\"Scroller();loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=view&value=".$id."&valuetype=".$do."');return false\"><img src=".$image." width=50 border=0></a></div><div style=\"margin-left:0px;float:left;width:80%;height:35px;border-radius:0px;padding-left:5px;padding-top:15px;padding-right:5px;padding-bottom:5px;\">".$tier."[".$parent_service_type_name." -> ".$service_type_name."]<BR>".$edit." <a href=\"#\" onClick=\"Scroller();loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=view&value=".$id."&valuetype=".$do."');return false\"><B>".$name."</B></a><BR>".$strings["Capacity"].": ".$service_capacity."".$sla_bundle."</div></div></div><BR><img src=images/blank.gif height=5 width=98%>";
   ...
   case 'view':
    ...
    if ($action == 'edit'){ 
       ...
       switch ($system_action_edit){
        ...
        break;
        case $action_rights_owner:
         $ci_params[0] = " id = '".$val."' && contact_id_c = '".$sess_contact_id."' ";
        break;
        case $action_rights_account:
         $ci_params[0] = " id = '".$val."' && account_id_c = '".$sess_account_id."' ";
        break;

       } // end switch system_action_view_list

       # Not worrying about the rights for now..
       $ci_params[0] = " id = '".$val."'";
    ...
    if ($ci_params[0] != NULL){
       ...
       if (is_array($ci_items)){
          ...
          for ($cnt=0;$cnt < count($ci_items);$cnt++){
              ...
              #if ($action == 'edit'){ 
                 $accountsservices_id_c = $ci_items[$cnt]['id'];
              #   }
...
*/
    ...
    $tablefields[$tblcnt][21] = $accountsservices_id_c; //$field_value;   
    ...
    if ($action == 'edit' || $action == 'add'){
       ...
       $ci_action = "select";
       $ci_params[0] = " account_id_c='".$sess_account_id."' && deleted=0 && content_type='1b7369c3-dd78-a8a3-2a79-523876ce70fe' && sclm_accountsservices_id_c='".$val."' ";
    ...
    #if ($service_type != NULL && $action == 'view'){
    ...
    /*
       ...
       if ($service_type == NULL){
          ...
          $tablefields[$tblcnt][9][4] = " (sclm_configurationitemtypes_id_c='cabebb61-5bef-f61c-fd3d-51d18355ccdb' || sclm_configurationitemtypes_id_c='1dea7f68-6f6a-662f-fb43-54f829549bd9') && sclm_configurationitems_id_c='NULL' ";

          } else {

          $tablefields[$tblcnt][9][4] = "id='".$service_type."' ";
...
#    if (($action == 'view' || $action == 'edit') && ($sclm_accountsservices_id_c != NULL || $sclm_accountsservices_id_c != 'NULL')){ 
   if (($action == 'view' || $action == 'edit') && $sclm_accountsservices_id_c != "NULL" && $sclm_accountsservices_id_c != NULL){ 
    ...
    $field_security_check = $funky_gear->check_security($field_security_params);
...
//       $tablefields[$tblcnt][9][4] = ""; // exception
    ...
    if ($action == 'add'){
       ...
       */
       ...
       # Manually adding as we create this - must be a better way to detect - maybe build a special array of types..
       $livestock_array['ec42e59e-efe3-6499-84ed-5767154bd677'] = 'Sheep';
       $service_types = array_merge($service_types,$livestock_array);
...
//          $sla_params[0] = $object_return_params[0];
    ...
    #echo "Auth: ".$auth." $sess_account_id == $ci_account_id_c  <P>";
    ...
    switch ($system_action_view){
      ...
      case ($action_rights_all || ($action_rights_owner && $sess_contact_id != NULL && $sess_contact_id==$ci_contact_id_c) || ($action_rights_account && $sess_account_id == $ci_account_id_c)):
    ...
    #echo $object_return;
    #echo "<BR><img src=images/blank.gif width=98% height=2><BR>";
...
//       $accserv_params[0] = " account_id_c='".$_SESSION['account_id']."' && sclm_services_id_c='".$sclm_services_id_c."' && id='".$val."' ";
    ...
    if (($action == 'edit' || $action == 'view') && $image != NULL){
       ...
       echo "<center><a href='".$image."' data-lightbox=\"".$valtype."\" title=\"".$name."\"><img src='".$image."' width=50%></a></center>";
       }
    ...
    if ($action == 'edit' || $action == 'view' ){
       ...
       if (is_array($acc_serv_items)){
          ...
          echo "<div style=\"".$custom_portal_divstyle."\"><center><font size=3 color=".$portal_font_colour."><B>Service Instances for Allocation</B></font></center></div>";
   ...
   case 'process':
    ...
    /*
       foreach ($_POST as $key=>$value){
               ...
               if (str_replace("sclm_servicessla_id_c_","",$key) != $key){
                  $sclm_servicessla_id_c = str_replace("sclm_servicessla_id_c_","",$key);
                  } else {
                  $sclm_servicessla_id_c = "";
                  }  

               if (str_replace("sclm_accountsservicesslas_id_c_","",$key) != $key){
               ...
               if ($value == 1 && $sclm_accountsservicesslas_id_c != NULL){
                  echo "ACC Serv SLA:".$sclm_accountsservicesslas_id_c."<BR>";
                  } elseif ($value == 0 && $sclm_accountsservicesslas_id_c != NULL){
                  # Existing one with value now set to closed
                  echo "ACC Serv SLA:".$sclm_accountsservicesslas_id_c." newly closed<BR>";
                  } elseif ($value == 1 && $sclm_servicessla_id_c != NULL){
                  # New one with value set to open
                  echo "New ACC Serv SLA based on Service SLA:".$sclm_servicessla_id_c." set to open<BR>";
                  ...
                  echo "Base SLA Name: ".$sla_name."<BR>";
                  }
...
//               if ($value == 1 && ($sclm_servicessla_id_c != NULL || $sclm_accountsservicesslas_id_c != NULL)){
               if (($sclm_servicessla_id_c != NULL && $value == 1) || ($sclm_accountsservicesslas_id_c != NULL && ($value == 1 || $value == 0))){
                  ...
                  echo "Value:".$value." <BR>";
                  echo "Serv SLA:".$sclm_servicessla_id_c." <BR>";
                  echo "ACC Serv SLA:".$sclm_accountsservicesslas_id_c." <BR>";
...
//               if ($value == 1 && ($sclm_servicessla_id_c != NULL || $sclm_accountsservicesslas_id_c != NULL)){
               if (($sclm_servicessla_id_c != NULL && $value == 1) || ($sclm_accountsservicesslas_id_c != NULL && ($value == 1 || $value == 0))){
                  ...
                  if ($accountsservicesslas_id_c != NULL){
                     ...
                     if ($acc_service_id != NULL && $service_type != NULL && ($sclm_servicessla_id_c != NULL && $value == 1)){
                        ...
                        if ($instances != NULL && $instances > 0){
                           ...
                           for ($accsla_cnt=0;$accsla_cnt < $instances;$accsla_cnt++){
                               ...
                               $acc_serv_process_params[] = array('name'=>'sclm_configurationitems_id_c','value' => $acc_service_id); # Becomes Parent
                           ...
                           } # if not null and more than zero
