...
# case 'AccountsServicesSLAs':
  ...
  if ($valtype == 'AccountsServices' && $val != NULL && ($action == 'add' || $action == 'list')){
  ...
  switch ($action){
   ...
   case 'list':
    ...
    if (($sess_account_id != NULL && $sess_account_id == $ci_account_id_c) || $auth == 3){
       $object_return_params[0] = " deleted=0 && sclm_accountsservices_id_c='".$sclm_accountsservices_id_c."'" ;
       } else {
       $object_return_params[0] = " deleted=0 && cmn_statuses_id_c !='".$standard_statuses_closed."' && sclm_accountsservices_id_c='".$sclm_accountsservices_id_c."' " ;
       }
...
*/
          #echo "$sess_account_id != NULL && $ci_account_id_c != NULL && $sess_account_id==$ci_account_id_c && $auth>1";
           ...
           if ($sess_account_id != NULL){
              ...
              # Check to see if Service Prices exist for account - any partner may add pricing for a service
              $sp_object_type = 'ServicesPrices';
              $sp_action = "select";
              $sp_params[0] = " sclm_servicessla_id_c='".$sclm_servicessla_id_c."' && account_id_c='".$sess_account_id."' ";
              $sp_params[1] = ""; // select array
              $sp_params[2] = ""; // group;
              $sp_params[3] = " sclm_services_id_c,sclm_servicessla_id_c,cmn_currencies_id_c,cmn_countries_id_c,cmn_languages_id_c,name, date_entered DESC ";
              $sp_params[4] = ""; // limit
  
              $sp_items = api_sugar ($crm_api_user, $crm_api_pass, $crm_wsdl_url, $sp_object_type, $sp_action, $sp_params);

              $addprice = "";

              if (is_array($sp_items)){

                 # Price exists - no need to provide add link

                 } else {

                 # Price doesn't exist - provide add link

                 $addprice = " <a href=\"#\" onClick=\"Scroller();loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=ServicesPrices&action=add&value=".$id."&valuetype=".$do."');return false\"><font size=2 color=red><B>[Add Partner Pricing]</B></font></a>";
...
//           $cis .= "<div style=\"".$divstyle_white."\">".$edit." [<a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=Services&action=list&value=".$parent_service_type_id."&valuetype=ConfigurationItems');return false\">".$parent_service_type_name." -></a> <a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=Services&action=list&value=".$service_type_id."&valuetype=ConfigurationItems');return false\">".$service_type_name."</a>] <a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=view&value=".$id."&valuetype=".$valtype."');return false\"><B>".$name."</B></a> [Capacity: ".$service_capacity."]</div>";
           $cis .= "<div style=\"".$divstyle_white."\">".$edit." <a href=\"#\" onClick=\"Scroller();loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=view&value=".$id."&valuetype=".$valtype."');return false\"><B>".$name."</B></a> [Capacity: ".$service_capacity."]".$own_capacity.$addprice."</div>";
    ...
    if ($sess_account_id != NULL && ($sess_account_id == $ci_account_id_c) && $auth>1 && $sclm_services_id_c != NULL){
       ...
       # Should only add new ones if no SLAs remain to be selected from
