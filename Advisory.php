...
//  $system_access = TRUE;
    ...
    if (is_array($advisory_items)){
       ...
       for ($cnt=0;$cnt < count($advisory_items);$cnt++){
           ...
           $image_ci_id = $advisory_items[$cnt]['name'];

           if ($image_ci_id != NULL){

              $advisoryimage_object_type = "ConfigurationItems";
              $advisoryimage_action = "select";
              $advisoryimage_params[0] = " id='".$image_ci_id."' ";
...
<center>
   <form action="javascript:get(document.getElementById('myform'));" name="myform" id="myform">
    <div>
     <input type="text" id="value" name="value" value="<?php echo $keyword; ?>" size="20" style="font-family: v; font-size: 10pt; color: #5E6A7B; border: 1px solid #5E6A7B; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1;"><BR>
...
/*    
    if ($valtype == 'search' || $val != NULL){
       ...
       echo "<div style=\"".$custom_portal_divstyle."\"><center><font size=3 color=".$portal_font_colour."><B>".$strings["Advisory"]."</B></font></center></div>";
...
*/
   ...
   case 'view':
    ...
    if ($action == 'edit' || $action == 'view' && $valtype != 'Advisory'){ 

       $object_returner = $funky_gear->object_returner ('Advisory', $val);
       echo $object_returner[1]; //$object_return;
       #echo "<img src=images/blank.gif width=98% height=3><BR>";

       }

    if ($action == 'add' || $action == 'edit'){ 

       echo "<center><input onclick=\"createEditor();\" type=\"button\" class=\"css3button\" value=\"".$strings["HTMLEditorShow"]."\"><input onclick=\"removeEditor();\" type=\"button\" class=\"css3button\" value=\"".$strings["HTMLEditorHide"]."\"><BR>".$strings["HTMLEditorCopyMessage"]."</center><P>";       echo "HTML can be used in the description fields.<P>";
    ...
    $container_params[3] = $strings["Advisory"].": ".$name; // container_title
    ...
    #if ($action == 'view' || $action == 'edit' && $sess_account_id == $account_id_c){
