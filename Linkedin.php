...
if ($action == NULL){
  ...
  switch ($action){
   ...
   case 'SetApp':
    ...
    if (is_array($ci_items)){          
       ...
       for ($cnt=0;$cnt < count($ci_items);$cnt++){
           ...
           if ($ci_type_id == '7f853e35-698d-1390-6ea9-551b660d9bbe'){ # 

              $li_app_id_cid = $ci_items[$cnt]['id'];
              $li_app_id = $ci_items[$cnt]['name'];

              } elseif ($ci_type_id == '42a70683-202e-5530-84cb-551b668d2c1b'){ # 

              $li_app_secret_cid = $ci_items[$cnt]['id'];
              $li_app_secret = $ci_items[$cnt]['name'];
    ...
    $tablefields[$tblcnt][0] = "li_app_id_cid"; // Field Name
    ...
    $tablefields[$tblcnt][9] = '';//$dropdown_table; // related table
    $tablefields[$tblcnt][10] = '';//1; // show in view 
    $tablefields[$tblcnt][11] = 'li_app_id_cid'; // Field ID
    $tablefields[$tblcnt][12] = 40; // Field ID
    $tablefields[$tblcnt][20] = $li_app_id_cid; //$field_value_id;
    $tablefields[$tblcnt][21] = $li_app_id_cid; //$field_value;
    $tblcnt++;

    $tablefields[$tblcnt][0] = "li_app_id"; // Field Name
    $tablefields[$tblcnt][1] = "APP ID"; // Full Name
    $tablefields[$tblcnt][2] = 0; // is_primary
    $tablefields[$tblcnt][3] = 0; // is_autoincrement
    $tablefields[$tblcnt][4] = 0; // is_name
    ...
    $tablefields[$tblcnt][9] = '';//$dropdown_table; // related table
    $tablefields[$tblcnt][10] = '1';//1; // show in view 
    $tablefields[$tblcnt][11] = 'li_app_id'; // Field ID
    $tablefields[$tblcnt][12] = 40; // Field ID
    $tablefields[$tblcnt][20] = $li_app_id; //$field_value_id;
    $tablefields[$tblcnt][21] = $li_app_id; //$field_value;

    $tblcnt++;

    $tablefields[$tblcnt][0] = "li_app_secret_cid"; // Field Name
    ...
    $tablefields[$tblcnt][9] = '';//$dropdown_table; // related table
    $tablefields[$tblcnt][10] = '1';//1; // show in view 
    $tablefields[$tblcnt][11] = 'li_app_secret_cid'; // Field ID
    $tablefields[$tblcnt][12] = 40; // Field ID
    $tablefields[$tblcnt][20] = $li_app_secret_cid; //$field_value_id;
    $tablefields[$tblcnt][21] = $li_app_secret_cid; //$field_value; 
  
    $tblcnt++;

    $tablefields[$tblcnt][0] = "li_app_secret"; // Field Name
    $tablefields[$tblcnt][1] = "APP Secret"; // Full Name
    $tablefields[$tblcnt][2] = 0; // is_primary
    $tablefields[$tblcnt][3] = 0; // is_autoincrement
    $tablefields[$tblcnt][4] = 0; // is_name
    ...
    $tablefields[$tblcnt][9] = '';//$dropdown_table; // related table
    $tablefields[$tblcnt][10] = '1';//1; // show in view 
    $tablefields[$tblcnt][11] = 'li_app_secret'; // Field ID
    $tablefields[$tblcnt][12] = 40; // Field ID
    $tablefields[$tblcnt][20] = $li_app_secret; //$field_value_id;
    $tablefields[$tblcnt][21] = $li_app_secret; //$field_value;   
   ...
   case 'SetAppProcess':
    ...
    $li_app_id_cid = $_POST['li_app_id_cid'];
    $li_app_id = $_POST['li_app_id'];

    $li_app_secret_cid = $_POST['li_app_secret_cid'];
    ...
    if ($_POST['li_app_id'] && $_POST['par_ci_id'] && $_POST['contact_id_c'] && $_POST['account_id_c']){

       $process_message = $strings["SubmissionSuccess"]."<P>";

       $process_params = "";
       $process_params = array();  
       $process_params[] = array('name'=>'id','value' => $_POST['li_app_id_cid']);
       $process_params[] = array('name'=>'name','value' => $_POST['li_app_id']);
       $process_params[] = array('name'=>'assigned_user_id','value' => $assigned_user_id);
       $process_params[] = array('name'=>'description','value' => $_POST['li_app_id']);
       ...
       $process_params[] = array('name'=>'sclm_configurationitemtypes_id_c','value' => '7f853e35-698d-1390-6ea9-551b660d9bbe');
       ...
       $process_message .= "LI App ID: ".$_POST['li_app_id']."<P>";

       }

    if ($_POST['li_app_secret'] && $_POST['par_ci_id'] && $_POST['contact_id_c'] && $_POST['account_id_c']){

       $process_params = "";
       $process_params = array();  
       $process_params[] = array('name'=>'id','value' => $_POST['li_app_secret_cid']);
       $process_params[] = array('name'=>'name','value' => $_POST['li_app_secret']);
       $process_params[] = array('name'=>'assigned_user_id','value' => $assigned_user_id);
       $process_params[] = array('name'=>'description','value' => $_POST['li_app_secret']);
       $process_params[] = array('name'=>'account_id_c','value' => $_POST['account_id_c']);
       $process_params[] = array('name'=>'contact_id_c','value' => $_POST['contact_id_c']);
       #$process_params[] = array('name'=>'sclm_scalasticachildren_id_c','value' => $_POST['child_id']);
       $process_params[] = array('name'=>'sclm_configurationitems_id_c','value' => $_POST['par_ci_id']);
       $process_params[] = array('name'=>'sclm_configurationitemtypes_id_c','value' => '42a70683-202e-5530-84cb-551b668d2c1b');
       $process_params[] = array('name'=>'cmn_statuses_id_c','value' => $standard_statuses_closed);

       $result = api_sugar ($crm_api_user2, $crm_api_pass2, $crm_wsdl_url2, $process_object_type, $process_action, $process_params);
       $process_message .= "LI Secret ID: ".$_POST['li_app_secret']."<P>";
