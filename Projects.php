   ...
   if ($action == 'list'){
      ...
      if ($portal_account_id != $sess_account_id){
         ...
         if ($shared_portal_projects_sql != NULL){
            $object_return_params[0] = " (".$shared_portal_projects_sql." || account_id_c='".$sess_account_id."' )";
            } else {# if ticketing
...
*/
    ...
    if (count($project_cstm_items)>10){
       $project_listings .= $addnew.$project_pack.$addnew;
       } else {
       $project_listings .= $project_pack.$addnew;
       }
    ...
    if (($valtype == 'Projects' || $valtype == NULL) && $action == 'list'){
       ...
       $container_params[3] = $strings["Projects"]; // container_title
