  ...
  switch ($action){
   ...
   case 'list':
    ...
    if ($valtype == 'Search' || $val != NULL){
       echo "<div style=\"".$formtitle_divstyle_grey."\"><center><font size=3><B>".$strings["SOWItems"]."</B></font></center></div>";
       echo "<img src=images/blank.gif width=50% height=5><BR>";
       echo "<center><img src=images/icons/Notes.png width=50 border=0 alt=\"".$strings["SOWItems"]."\"></center>";
       echo "<img src=images/blank.gif width=50% height=5><BR>";
       }
    ...
    if (is_array($sow)){
       ...
       $looparams[0] = $do;
       $looparams[1] = $sow;
       $looparams[2] = 'sclm_sow_id_c';

//       $sowitems = $funky_gear->looper ($looparams);
...
//       echo $sowitems[0];
    ...
    if (is_array($sow)){
       ...
       for ($cnt=0;$cnt < count($sow);$cnt++){
           ...
           $sclm_sow_id_c = $sow[$cnt]['sclm_sow_id_c'];
           $sclm_sowitems_id_c = $sow[$cnt]['sclm_sowitems_id_c'];
           $grandparent_sclm_sowitems_id_c = $sow[$cnt]['parent_sclm_sowitems_id_c'];
           $item_number = $sow[$cnt]['item_number'];
           ...
           if ($grandparent_sclm_sowitems_id_c != NULL){
              $grandparent_sowitem_returner = $funky_gear->object_returner ('SOWItems', $grandparent_sclm_sowitems_id_c);
              $grandparent_sowitem_object_return_name = $grandparent_sowitem_returner[0];
              $grandparent_sowitem_link = $grandparent_sowitem_returner[3];
              $grandparent_sowitem = $grandparent_sowitem_link." -> ";
              }

           if ($sclm_sowitems_id_c != NULL){
              $sowitem_returner = $funky_gear->object_returner ('SOWItems', $sclm_sowitems_id_c);
              $sowitem_object_return_name = $sowitem_returner[0];
              $sowitem_link = $sowitem_returner[3];
              $parent_sowitem = $grandparent_sowitem.$sowitem_link." -> ";
              }

           $sows .= "<div style=\"".$divstyle_white."\"> ".$edit." ".$parent_sowitem."<a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=view&value=".$id."&valuetype=".$valtype."');return false\"><B>[".$item_number."] ".$name."</B></a></div>";
...
//    $this->funkydone ($_POST,$lingo,'Content','view','b48bd8cb-33bc-66b8-1aa5-525477be579a','Projects',$bodywidth);
   ...
   case 'view':
    ...
    if ($action == 'edit' || $action == 'view'){ 
       ...
       if (is_array($sow)){
          ...
          for ($cnt=0;$cnt < count($sow);$cnt++){
              ...
              $sclm_sow_id_c = $sow[$cnt]['sclm_sow_id_c'];
              $sclm_sowitems_id_c = $sow[$cnt]['sclm_sowitems_id_c'];
              $parent_sclm_sowitems_id_c = $sow[$cnt]['parent_sclm_sowitems_id_c'];
              $item_number = $sow[$cnt]['item_number'];
    ...
    $tablefields[$tblcnt][0] = "item_number"; // Field Name
    ...
    $tablefields[$tblcnt][10] = '1';//1; // show in view 
    $tablefields[$tblcnt][11] = ""; // Field ID
    $tablefields[$tblcnt][20] = "item_number"; //$field_value_id;
    $tablefields[$tblcnt][21] = $item_number; //$field_value;   
  ...
  if ($valtype == 'SOW' && $val != NULL){
    ...
    if ($parent_sclm_sowitems_id_c != NULL && $action == 'view'){
       ...
       $tablefields[$tblcnt][9][3] = 'name';
       $tablefields[$tblcnt][9][4] = " account_id_c='".$account_id_c."' ";// exception;
       $tablefields[$tblcnt][9][5] = $parent_sclm_sowitems_id_c; // Current Value
       ...
       $tablefields[$tblcnt][9][9] = $parent_sclm_sowitems_id_c; // Current Value
       $tablefields[$tblcnt][10] = '1';//1; // show in view 
       $tablefields[$tblcnt][11] = ""; // Field ID
       $tablefields[$tblcnt][20] = 'sclm_sowitems_id_c';//$field_value_id;
       $tablefields[$tblcnt][21] = $parent_sclm_sowitems_id_c; //$field_value;
    ...
    $container_params[3] = $strings["SOWItem"].": ".$name; // container_title
...
*/
    if ($action == 'view'){
       ...
       echo "<center><a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=AccountsServices&action=list&value=".$parent_account_id."&valuetype=Parent&source=".$do."&sourceval=".$val."');return false\" class=\"css3button\"><B>".$strings["AttachServicesSOWItem"]."</B></a></center>";
//       echo "<center><a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=AccountsServices&action=list&value=".$val."&valuetype=".$do."');return false\" class=\"css3button\"><B>".$strings["AttachServicesSOWItem"]."</B></a></center>";
       echo "<BR><img src=images/blank.gif width=200 height=15><BR>";
...
*/
   ...
   case 'process':
    ...
    if (!$error){
       ...
       $process_params[] = array('name'=>'item_number','value' => $_POST['item_number']);
