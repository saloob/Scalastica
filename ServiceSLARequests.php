...
//       $addnew = "<div style=\"".$divstyle_blue."\"><a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=add&value=".$val."&valuetype=".$valtype."');return false\"><font color=#151B54><B>".$strings["action_addnew"]."</B></font></a></div>";
    ...
    $sla_listings .= $services;
    $sla_listings .= "<div style=\"".$divstyle_blue."\" id=SLAs name=SLAs></div>";

    if (count($ci_items)>10){
       $sla_listings .=  $addnew.$cis_on.$addnew;
       $sla_listings .=  $addnew.$cis_off.$addnew;
       } else {
       $sla_listings .=  $cis_on.$addnew;
       $sla_listings .=  $cis_off.$addnew;
       }
   
    $sla_listings .=  $navi;

#    if (($valtype == 'ServiceSLARequests' || $valtype == NULL) && $action == 'list'){
       ...
       $container_params[4] = 'ServiceSLARequests'; // container_label
       ...
       #echo "<BR><img src=images/blank.gif width=90% height=5><BR>";
       #echo $container_top;
       $sla_header = "<BR><img src=images/blank.gif width=90% height=5><BR>";
    ...
    $sla_header .= <<< SLASEARCH
<center>
   <form action="javascript:get(document.getElementById('myform'));" name="myform" id="myform">
    <div>
     <input type="text" id="keyword" placeholder="$action_search_keyword" name="keyword" value="$keyword" size="20">
...
SLASEARCH;
       ...
       $sla_header .= "<center><div id=\"clockDisplay\" class=\"clockStyle\"></div></center>";
       $sla_header .= "<BR><img src=images/blank.gif width=90% height=5><BR>";
       $sla_header .= "<div style=\"".$divstyle_orange."\">".$strings["ServiceSLARequestMessage"]."</div>";
...
*/
    ...
    #echo "Query :".$tablefields[$tblcnt][9][4]."<P>";
    ...
    if ($sclm_servicessla_id_c != NULL){
       $tablefields[$tblcnt][9][4] = " sclm_servicessla_id_c='".$sclm_servicessla_id_c."' ";//$exception;
...
//    $tablefields[$tblcnt][42] = '1'; //no label   
    ...
    if ($action == 'add' || $action == 'edit '){
       ...
       if ($global_status){
          $global_query = " cmn_statuses_id_c != '".$standard_statuses_closed."' && cmn_statuses_id_c IS NOT NULL && cmn_statuses_id_c != '' ";
          } else {
          $global_query = " account_id_c='".$portal_account_id."' && cmn_statuses_id_c IS NOT NULL && cmn_statuses_id_c != '' ";
          }
