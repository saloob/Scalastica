...
# case 'ServicesPrices':
  ...
  if ($valtype == 'AccountsServicesSLAs'){
     ...
     $ci_params[1] = "sclm_services_id_c,account_id_c,sclm_servicessla_id_c,sclm_accountsservices_id_c"; // select array
     $ci_params[2] = ""; // group;
     $ci_params[3] = ""; // order;
     $ci_params[4] = ""; // limit
...
*/
           ...
           if ($sess_account_id == $portal_account_id && $sess_account_id != $ci_account_id_c){
              ...
              if (is_array($accrel_items)){              
                 ...
                 $provider_link .= "<a href=\"#\" onClick=\"Scroller();loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=AccountRelationships&action=add&value=".$ci_account_id_c."&valuetype=".$do."');return false\"><font size=2 color=red><B>[Make Partner]</font></B></a> -> ";
           ...
           if ($ci_contact_id_c != NULL && $sess_contact_id != NULL){
              ...
              $sendmessage = "<BR>Contact this provider: <a href=\"#Top\" onClick=\"loader('lightform');document.getElementById('lightform').style.display='block';doBPOSTRequest('lightform','Body.php', 'pc=".$portalcode."&do=Messages&action=add&value=".$ci_contact_id_c."&valuetype=Contacts&sendiv=light&related=ServicesPrices&relval=".$id."&sendiv=lightform');document.getElementById('fade').style.display='block';return false\"><img src=images/icons/MessagesIcon-100x100.png width=16 alt='".$strings["Message"]."'>".$agent_name."</a> ";
...
//           $cis .= "<div style=\"".$divstyle_white."\">".$edit." [<a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=Services&action=list&value=".$parent_service_type_id."&valuetype=ConfigurationItems');return false\">".$parent_service_type_name." -></a> <a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=Services&action=list&value=".$service_type_id."&valuetype=ConfigurationItems');return false\">".$service_type_name."</a>] <a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=view&value=".$id."&valuetype=".$valtype."');return false\"><B>".$name."</B></a> [Capacity: ".$service_capacity."]</div>";
           ...
           if ($sess_account_id != NULL && $valtype == 'AccountsServices'){
              $request = "<BR><a href=\"#\" onClick=\"Scroller();loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=ServiceSLARequests&action=add&value=".$id."&valuetype=".$do."&sourcevaltype=".$valtype."&sourceval=".$val."');return false\"><font color=red><B>Request this SLA</B></font></a> ";
          ...
          switch ($auth){
           ...
           $cis .= "<div style=\"".$divstyle_white."\">".$edit." ".$provider_link."<a href=\"#\" onClick=\"Scroller();loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=view&value=".$id."&valuetype=".$do."');return false\"><B>".$account_name.$name."</B></a>".$show_value.$request." ".$sendmessage."</div>";
    ...
    if ($sess_contact_id == NULL){
       echo "<div style=\"".$divstyle_orange."\"><center><B>".$strings["ServicesPrices_RequireLogin"]."</B></center></div>";
       }
...
//    $tablefields[$tblcnt][9][1] = 'sclm_servicessla,sclm_servicesprices'; // If DB, dropdown_table, if List, then array, other related table
    ...
    if ($action == 'add' && $sclm_servicessla_id_c != NULL){

       $tablefields[$tblcnt][9][4] = " id='".$sclm_servicessla_id_c."' ";

       } elseif ($action == 'add' && $sclm_services_id_c != NULL){
