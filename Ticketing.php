...
#    $ticket_return = " <center><a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=list&value=".$val."&valuetype=".$valtype."');return false\"><font size=4><B>".$strings["Tickets"]."</B></font></a> || <a href=\"#\" onClick=\"loader('GRID');doBPOSTRequest('GRID','Body.php', 'pc=".$portalcode."&do=".$do."&action=gridlist&value=".$val."&valuetype=".$valtype."&sendiv=GRID');return false\"><font size=4><B>".$strings["Tickets"]." [Grid]</B></font></a></center> ".$rowmaker;
   ...
   if ($action == 'search'){
      ...
      if ($search_keyword != NULL){
         ...
         $ticket_params[0] .= " && (description like '%".$search_keyword."%' || name like '%".$search_keyword."%' )";
         }
...
*/
               ...
               if ($lang == 'ja'){
                  $status_images .= "<img src=".$status_image." width=16> <a href=\"#\" onClick=\"loader('".$thisdiv."');doBPOSTRequest('".$thisdiv."','Body.php', 'pc=".$portalcode."&do=Ticketing&action=search&value=".$status."&valuetype=status&search_date=".$search_date."');return false\">".$status_name_ja."[".$statuscount."]</a> | ";
                  } else {
                  $status_images .= "<img src=".$status_image." width=16> <a href=\"#\" onClick=\"loader('".$thisdiv."');doBPOSTRequest('".$thisdiv."','Body.php', 'pc=".$portalcode."&do=Ticketing&action=search&value=".$status."&valuetype=status&search_date=".$search_date."');return false\">".$status_name."[".$statuscount."]</a> | ";
                  } 
               } // if statuses
...
//        $chart2->getChart()->getHeader()->setText($strings["TicketSummaryByStatus"]);
   ...
   case 'search':
    ...
    if ($action == 'gridlist' || $sendiv == 'GRID'){
       ...
       # Key = delete_ticket_XXXXXXXXXXXXXXXXXXX
       foreach ($_POST as $multi_key=>$multi_val){

               if (str_replace("delete_ticket_","",$multi_key) != $multi_key){
                  $to_delete_ticket_id = str_replace("delete_ticket_","",$multi_key);
                  } else {
                  $to_delete_ticket_id = "";
                  }  

               if ($to_delete_ticket_id == $multi_val && $to_delete_ticket_id != NULL){
                  ...
                  $process_params[] = array('name'=>'id','value' => $to_delete_ticket_id);
                  ...
                  if ($result['id'] != NULL){
                     echo "Record deleted successfully [Ticket ID: ".$to_delete_ticket_id."]<BR>";
                     }

                  } # is del/val

               if (str_replace("cancel_ticket_","",$multi_key) != $multi_key){
                  $to_cancel_ticket_id = str_replace("cancel_ticket_","",$multi_key);
                  } else {
                  $to_cancel_ticket_id = "";
                  }  

               if ($to_cancel_ticket_id == $multi_val && $to_cancel_ticket_id != NULL){

                  $process_object_type = $do;
                  $process_action = "update";
                  $process_params = "";
                  $process_params = array();  
                  $process_params[] = array('name'=>'id','value' => $to_cancel_ticket_id);
                  $process_params[] = array('name'=>'status','value' => '72b33850-b4b0-c679-f988-52c2eb40a5de');

                  $result = api_sugar ($crm_api_user2, $crm_api_pass2, $crm_wsdl_url2, $process_object_type, $process_action, $process_params);

                  if ($result['id'] != NULL){
                     echo "Record cancelled successfully [Ticket ID: ".$to_cancel_ticket_id."]<BR>";
                     }
    ...
    if (is_array($ticket_items)){
       ...
       $extraparams[0] = "&search_date=".$search_date."&keyword=".$search_keyword."&ticket_id=".$search_ticket_id."&service_operation_process=".$service_operation_process."&status=".$sent_status;
       ...
       $DateLabel = $strings["Date"].": ".$search_date; 
...
<center>
   <form action="javascript:get(document.getElementById('myform'));" name="myform" id="myform">
    <div>
     <input type="text" id="ticket_id" placeholder="<?PHP echo $ticket_label; ?> ID"  name="ticket_id" value="<?php echo $search_ticket_id; ?>" size="20" style="font-family: v; font-size: 10pt; color: #5E6A7B; border: 1px solid #5E6A7B; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1;">
     <input type="text" id="keyword" placeholder="<?PHP echo $action_search_keyword; ?>" name="keyword" value="<?php echo $search_keyword; ?>" size="20" style="font-family: v; font-size: 10pt; color: #5E6A7B; border: 1px solid #5E6A7B; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1;">
      <?php
       if ($action == 'list'){
          echo "<BR>";
          }
      ?>
     <input type="text" id="search_date" placeholder="<?PHP echo $DateLabel; ?>" name="search_date" value="<?php echo $search_date; ?>" size="20" style="font-family: v; font-size: 10pt; color: #5E6A7B; border: 1px solid #5E6A7B; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1;">
     <input type="hidden" id="pg" name="pg" value="<?php echo $body_sendvars; ?>" >
     <input type="hidden" id="action" name="action" value="search" >
     ...
     <input type="hidden" id="valuetype" name="valuetype" value="<?php echo $valtype; ?>" >
      <?php
       if ($action == 'gridlist' || $sendiv == 'GRID'){
      ?>
     <input type="button" name="button" value="<?php echo $strings["action_search"]; ?>" onclick="javascript:loader('GRID');get(this.parentNode);" style="font-family: v; font-size: 10pt; color: #5E6A7B; border: 1px solid #5E6A7B; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1;">
      <?php
          } else {
      ?>
     <input type="button" name="button" value="<?php echo $strings["action_search"]; ?>" onclick="javascript:loader('<?php echo $BodyDIV; ?>');get(this.parentNode);" style="font-family: v; font-size: 10pt; color: #5E6A7B; border: 1px solid #5E6A7B; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1;">
      <?php
          }
      ?>
...
*/
           ...
           $cancel = "<a href=\"#\" onClick=\"if(confirmer('Cancel?')){doBPOSTRequest('".$id."','Body.php', 'pc=".$portalcode."&do=Ticketing&action=cancel&value=".$id."&valuetype=".$do."');return false}\"><font size=2 color=red><B>[".$strings["Cancel"]."]</B></font></a> ";
           ...
           if ($ci_contact_id1_c){
              ...
              $sendmessage = " <a href=\"#Top\" onClick=\"loader('lightform');document.getElementById('lightform').style.display='block';doBPOSTRequest('lightform','Body.php', 'pc=".$portalcode."&do=Messages&action=add&value=".$ci_contact_id1_c."&valuetype=Contacts&sendiv=lightform&related=Ticketing&relval=".$id."');document.getElementById('fade').style.display='block';return false\"><img src=images/icons/MessagesIcon-100x100.png width=16 alt='".$strings["Message"]."'>".$agent_name."</a> ";
              }
           ...
           switch ($status){
            ...
            case 'e47fc565-c045-fef9-ef4f-52802bfc479c': //Open - Unclaimed
             ...
             #$addactivity = "";
             $delete = "<a href=\"#\" onClick=\"if(confirmer('Delete?')){doBPOSTRequest('".$id."','Body.php', 'pc=".$portalcode."&do=Ticketing&action=delete&value=".$id."&valuetype=".$do."');return false}\"><font size=2 color=red><B>[".$strings["Delete"]."]</B></font></a> ";
...
<B>".$strings["action_view"].":</B> <a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=view&value=".$id."&valuetype=".$valtype."');loader('".$RightDIV."');doRPOSTRequest('".$RightDIV."','Body.php','pc=".$portalcode."&do=".$do."&action=reference&value=".$id."&valuetype=".$valtype."');return false\"><B>".$name."</B></a>
       ...
       $body_sendvars = $encdate."#Bodyphp";
       $body_sendvars = $funky_gear->encrypt($body_sendvars);
       $action_search = $strings["Delete"];

?>
   <form action="javascript:get(document.getElementById('myform'));" name="myform" id="myform">
    <input type="hidden" id="sendiv" name="sendiv" value="GRID" >
    <input type="hidden" id="do" name="do" value="<?php echo $do; ?>" >
    <input type="hidden" id="action" name="action" value="gridlist" >
    <input type="hidden" id="value" name="value" value="<?php echo $val; ?>" >
    <input type="hidden" id="valuetype" name="valuetype" value="<?php echo $valtype; ?>" >
<?PHP
              ...
              $tickets .= "<div style=\"".$div_title."\">".$title."<BR>".$sdm_edits.$quick_edits.$quick_activity."<BR>".$edit.$delete.$cancel.$addactivity.$actcount.$sendmessage."<BR>Delete <input type=\"checkbox\" name=\"delete_ticket_".$id."\" id=\"delete_ticket_".$id."\" value=\"".$id."\"\"> OR Cancel <input type=\"checkbox\" name=\"cancel_ticket_".$id."\" id=\"cancel_ticket_".$id."\" value=\"".$id."\"\"></div>";
              $tickets .= "<div style=\"".$div_filter."\">".$filter."</B></div>";
              $tickets .= "</div><BR><img src=images/blank.gif width=98% height=1><BR>"; 
...
#       echo $addnew."<P>".$tickets;
    ...
    if ($action == 'gridlist' || $sendiv == 'GRID'){
       echo "<div style=\"".$formtitle_divstyle_white."\">".$ticketheader."</div><BR><img src=images/blank.gif width=98% height=1><BR>";
       }

    echo "<div style=\"".$formtitle_divstyle_white."\">".$tickets."</div>";

   # If any tickets selected for deletion
   ?>
    <input type="button" name="button" value="Managed Selected Tickets" onclick="if(confirmer('Proceed?')){javascript:loader('GRID');get(this.parentNode);return false}" style="font-family: v; font-size: 10pt; color: #5E6A7B; border: 1px solid #5E6A7B; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1;">
   </form>
<?PHP
       ...
       for ($cnt=0;$cnt < count($ticket_items);$cnt++){
           ...
           if ($ticket_update != NULL){
              ...
              if ($ci_contact_id1_c){
                 $agent_returner = $funky_gear->object_returner ("Contacts", $ci_contact_id1_c);
                 $agent_name = $agent_returner[0];
                 $sendmessage = " <a href=\"#Top\" onClick=\"loader('lightform');document.getElementById('lightform').style.display='block';doBPOSTRequest('lightform','Body.php', 'pc=".$portalcode."&do=Messages&action=add&value=".$ci_contact_id1_c."&valuetype=Contacts&sendiv=light&related=Ticketing&relval=".$val."&sendiv=lightform');document.getElementById('fade').style.display='block';return false\"><img src=images/icons/MessagesIcon-100x100.png width=16 alt='".$strings["Message"]."'>".$agent_name."</a> ";
                 }


              $ticket_update = $funky_gear->replacer("\n", "<BR>", $ticket_update);

              $activities .= "<div style=\"".$divstyle_white."\">".$sendmessage." ".$icon_star." <a href=\"#\" onClick=\"loader('light');document.getElementById('light').style.display='block';doBPOSTRequest('light','Body.php', 'pc=".$portalcode."&do=TicketingActivities&action=view&value=".$id."&valuetype=".$do."&sendiv=light');return false\"><font size=2 color=black>".$ticket_update."</font></a></div>";
...
<center>
   <form action="javascript:get(document.getElementById('myform'));" name="myform" id="myform">
     <input type="text" id="ticket_id" placeholder="$ticket_label ID" name="ticket_id" value="$search_ticket_id" size="20" style="font-family: v; font-size: 10pt; color: #5E6A7B; border: 1px solid #5E6A7B; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1;">
...
TICKSEARCH;
    ...
    echo "<div style=\"".$custom_portal_divstyle."\"><center><B><font size=3 color=".$portal_font_colour.">".$strings["Updates"]."</B></center></div>";
    echo $activities;
  ...
  if (!$ci_contact_id1_c){
    ...
    $valpack[4] = $auth; // $auth; // user level authentication (3,2,1 = admin,client,user)
    $valpack[5] = ""; // provide add new button
    $valpack[6] = "quick_process"; // custom action
    $valpack[7] = $strings["Edit"];
