...
# Page: api.php 
##########################################################
# case 'Product_categoriesController':

mb_language('uni');
mb_internal_encoding('UTF-8');

class Product_categoriesController extends AbstractController
{
     ...
     global $funky_gear,$assigned_user_id,$portal_email_server,$portal_account_id,$portal_email_password,$portal_email,$portal_title,$hostname,$db_host,$db_name,$db_user,$db_pass,$strings,$lingo,$lingoname,$divstyle_white,$divstyle_grey,$divstyle_blue,$divstyle_orange,$crm_api_user,$crm_api_pass,$crm_wsdl_url,$BodyDIV,$portalcode,$crm_api_user2, $crm_api_pass2, $crm_wsdl_url2,$account_id_c,$contact_id_c,$cmn_languages_id_c,$cmn_statuses_id_c,$cmn_countries_id_c,$standard_statuses_closed;
      ...
      switch ($action){
       ...
       case 'list':
        ...
        $par_category_id = $request->parameters['parent_category_id'];
        ...
        # Product Items | ID: 1dea7f68-6f6a-662f-fb43-54f829549bd9
        # Product Services | ID: cabebb61-5bef-f61c-fd3d-51d18355ccdb
        ...
        */
        ...
        #$cat_params[0] .= " && (sclm_configurationitemtypes_id_c='cabebb61-5bef-f61c-fd3d-51d18355ccdb' || sclm_configurationitemtypes_id_c='1dea7f68-6f6a-662f-fb43-54f829549bd9') && sclm_configurationitems_id_c='NULL'";

        $cat_params[0] .= " && (sclm_configurationitemtypes_id_c='cabebb61-5bef-f61c-fd3d-51d18355ccdb' || sclm_configurationitemtypes_id_c='1dea7f68-6f6a-662f-fb43-54f829549bd9') ";

        if ($category_id != NULL){
           $cat_params[0] .= " && id='".$category_id."' ";
           } elseif ($par_category_id != NULL){
           $cat_params[0] .= " && sclm_configurationitems_id_c='".$par_category_id."' ";
        ...
        if (is_array($catpar_items)){
           ...
           for ($catparcnt=0;$catparcnt < count($catpar_items);$catparcnt++){
               ...
               $cat_parent_id = $catpar_items[$catparcnt]['sclm_configurationitems_id_c'];
               #$cat_pack[$category_id] = $cat_name;
               $cat_pack[$catparcnt]['id'] = $category_id;
               $cat_pack[$catparcnt]['name'] = $cat_name;
               $cat_pack[$catparcnt]['parent_category_id'] = $cat_parent_id;

               if ($cat_parent_id != NULL){
                  $parcat_returner = $funky_gear->object_returner ("ConfigurationItems", $cat_parent_id);
                  $parent_category_name = $parcat_returner[0];
                  }

               $cat_pack[$catparcnt]['parent_category_name'] = $parent_category_name;
