...
TTeeChartPropertyEditor = class(TArrayPropertyEditor)
...
{
The only method to override is beforeShowEditor, which is called to prepare the editor for working,
the rest of methods are helpers and event handlers for the controls in the editor.
Implementing the methods
The first method to implement is beforeShowEditor, the dialog parameter contains the form that is
shown to the user, so we can customize it:
}

procedure TTeeChartPropertyEditor.beforeShowEditor(dialog: TForm);
begin
inherited;
...
{
On the first lines, we set the caption for the editor and the buttons that are going to be available.
Also, we set which property, of the existing attributes, is going to be used to set the caption of the entry,
in this case we set it to Name, so when the user changes that value on the right side of the editor, the name
of the serie will be changed.

Later, all the default properties for a serie are added, with its default values, and finally, the event handlers
are attached.

Setting the default name for new series
}

procedure TTeeChartPropertyEditor.GetNewItemCaption(Sender: TObject;
...
{
This method is called when a new item is created by the user, in this case, a new serie, it just returns on
itemcaption the name of the serie with a sequence number.

Creating events from the property editor
}

procedure TTeeChartPropertyEditor.vePropertiesDblClick(Sender: TObject);
var
ename: string;
control: string;
x, y: integer;
...
{
And finally, this method is called when the user drops down the value box on the values editor,
so we can offer a list of valid values. In this case, for Type, we offer all the types of charts
the user can create, for Animate, just true or false, and for DataSource, we show a list of Datasource
components found on the form. The getcomponents function is a built-in function offered by the IDE.

Initialization
To attach this property editor to the Series property of the TChartObj component, just use the initialization
section this way:
...
registerPropertyEditor('TeeChart', 'Editor', TTeeChartPropertyEditor);
