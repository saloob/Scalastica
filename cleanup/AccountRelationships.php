...
# case 'AccountRelationships':
  ...
  if (!$val){
     $ci_params[0] = " account_id_c='".$sess_account_id."' || account_id1_c='".$sess_account_id."' ";
     } else {
     $ci_params[0] = " account_id_c='".$sess_account_id."' || account_id1_c='".$val."' ";
     }
...
*/
           ...
           $cis .= "<div style=\"".$divstyle_grey."\">".$edit."<a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=".$do."&action=view&value=".$id."&valuetype=".$do."');return false\"><B>".$name."</B></a>".$show_id."</div>";
           ...
           if ($valtype == 'Contacts'){
              ...
              for ($relcnt=0;$relcnt < count($items_list);$relcnt++){
                  ...
                  if (($sess_contact_id == $rel_contact_id && $sess_contact_id != NULL) || $auth==3 || $is_admin==TRUE){
                     ...
                     $rel_edit = "<a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=Contacts&action=edit&value=".$rel_contact_id."&valuetype=Contacts');return false\"><font color=RED><B> [".$strings["action_edit"]."] </B></font></a>";
                  ...
                  $sendmessage = "";
                  $sendmessage = "<a href=\"#Top\" onClick=\"loader('light');document.getElementById('light').style.display='block';doBPOSTRequest('light','Body.php', 'pc=".$portalcode."&do=Messages&action=add&value=".$rel_contact_id."&valuetype=Contacts&sendiv=light&related=".$valtype."&relval=".$val."');document.getElementById('fade').style.display='block';return false\"><img src=images/icons/MessagesIcon-100x100.png width=16 alt='".$strings["Message"]."'></a>";

                  $rel_contacts .= $rel_edit."<a href=\"#Top\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','Body.php', 'pc=".$portalcode."&do=Contacts&action=view&value=".$rel_contact_id."&valuetype=Contacts');return false\"><font color=#151B54>".$rel_first_name." ".$rel_last_name."</font></a> ".$sendmessage."<BR>";
