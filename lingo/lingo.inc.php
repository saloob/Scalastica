  ...
  for ($x=0;$x<count($lingos);$x++) {
      ...
      if ($lingo == $extension){
         ...
         if ($LeftDIV != NULL && $LeftDIV != NULL){
            ...
            $language_bit = "<a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','tr.php','pc=$portalcode&sv=$sendvars');loader('".$LeftDIV."');doLPOSTRequest('".$LeftDIV."','tr.php','pc=$portalcode&sv=$left_sendvars');loader('".$RightDIV."');doRPOSTRequest('".$RightDIV."','tr.php','pc=$portalcode&sv=$right_sendvars');timedRefresh(10000);return false\"><img src=".$image." border=0 width=18 height=12 alt='".$language."'></a>";
            } else {

            $sendvars = "Body@".$extension."@".$do."@".$action."@".$val."@".$valtype;
            $sendvars = $funky_gear->encrypt($sendvars);

            $language_bit = "<a href=\"#\" onClick=\"loader('".$BodyDIV."');doBPOSTRequest('".$BodyDIV."','tr.php','pc=$portalcode&sv=$sendvars');timedRefresh(10000);return false\"><img src=".$image." border=0 width=18 height=12 alt='".$language."'></a>";
            } 
