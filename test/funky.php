...
(!preg_match('/^(\\\\.|[A-Za-z0-9!#%&`_=\\/$\'*+?^{}|~.-])+$/',
 ...
 function do_rego ($passed_params){
  ...
  # Start the messaging
  $process_message .= "<P>Welcome to the ".$portal_title." registration, ".$first_name."!<P>";
  $process_message .= "Your ".$service_name." email is: ".$email."<BR>";
  $process_message .= "Your ".$service_name." ID is: ".$userid."<P>";
  ...
  if ($sc_session != NULL && $sess_var != "scalastica"){
     ...
     $process_message .= "Your ".$portal_title." account exists with us already - welcome back!<P>";
     $process_message .= "We will now try to link your ".$service_name." account with this one...<P>";
     $syncuid = $sc_session;
  ...
  if ($fb_session != NULL && $sess_var != "facebook"){
     ...
     # There is a FB session, but the user wants to connect another account to the FB one!
     $process_message .= "Your Facebook account exists with ".$portal_title." already - welcome back!<P>";
     $process_message .= "We will now try to link your ".$service_name." account with this one...<P>";
     $syncuid = $fb_session;
  ...
  if ($li_session != NULL && $sess_var != "linkedin"){
     ...
     $process_message .= "Your LinkedIn account exists with ".$portal_title." already - welcome back!<P>";
     $process_message .= "We will now try to link your ".$service_name." account with this one...<P>";
     $syncuid = $li_session;
  ...
  if ($gg_session != NULL && $sess_var != "google"){
     ...
     $process_message .= "Your Google account exists with ".$portal_title." already - welcome back!<P>";
     $process_message .= "We will now try to link your ".$service_name." account with this one...<P>";
     $syncuid = $gg_session;
  ...
  if ($syncuid != NULL){
     ...
     switch ($sess_var){
      ...
      case 'google':
       ...
       $process_message .= "Your account with ".$service_name." has now been linked to ".$portal_title." - welcome!<P>";
       $process_message .= "<input type=\"button\" style=\"font-family: v; font-size: 10pt; background-color: #1560BD; border: 1px solid #5E6A7B; border-radius:10px; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1; width:150px;color:#FFFFFF;\" value=\"".$strings["action_clicktologin"]."\" onClick=\"location.href='api-google.php/?gguid=".$userid."'\"><P>";
      break;
      case 'facebook':
       $_SESSION['facebook'] = $userid;
       $process_message .= "Your account with ".$service_name." has now been linked to ".$portal_title." - welcome!<P>";
       $process_message .= "<input type=\"button\" style=\"font-family: v; font-size: 10pt; background-color: #1560BD; border: 1px solid #5E6A7B; border-radius:10px; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1; width:150px;color:#FFFFFF;\" value=\"".$strings["action_clicktologin"]."\" onClick=\"location.href='../'\"><P>";
      break;
      case 'linkedin':
  ...
  if (is_array($syncrego_rows) && $syncuid == NULL){
     ...
     # This user exists for this source (FB)
     $process_message .= "Your ".$service_name." account exists with ".$portal_title." already - welcome back!<P>";
     ...
     $process_message .= "Your ".$service_name." account does not exist with us - we will check your email (".$email.") from another service...<P>";
     ...
     if ($rego_contact_id_c != NULL){
        ...
        $process_message .= "Your ".$service_name." email (".$email.") exists with us already - we will now link it to ".$portal_title."...<P>";
        ...
        $process_message .= "Your ".$service_name." account and email (".$email.") do not exist with ".$portal_title." - we will set it up now...<P>";
           ...
           if ($rego_account_id_c != NULL){
              ...
              if ($rego_hostname != NULL && $rego_hostname_type != NULL){
                 ...
                 if (is_array($hn_rows)){
                    ...
                    # Already exists!
                    $process_message .= "The hostname (".$rego_hostname.") you selected already exists - it must be unique - please try again from the Account Details menu...<P>";

                    } else {
   
                    $process_message .= "The hostname (".$rego_hostname.") you selected does not already exist with us - please be sure to set your DNS A Records to point at the IP of ".$hostname." [119.27.35.191] - unless you have selected our sub-domain, in which case we will notify you when is is ready. This can later be adjusted in the Account Details menu...<P>";
                 ...
                 $process_message .= "No hostname was selected for this account. This can be adjusted from the Account Details menu...<P>";
              ...
              if (is_array($ci_items)){
                 ...
                 $process_message .= "Portal access to ".$service_name." has been set - see Account Details for other features...<P>";

                 } # end is_arrayi_items

                $process_message .= "Your ".$service_name." account and email (".$email.") have now been linked to ".$portal_title." - welcome!<P>";
             ...
             Enyoy using ".$portal_title."!";
              ...
              if ($emailresult[0] == 'OK'){
                 ...
                 $process_message .= "Please check your email to get your password - it has been sent there.<P>";
              ...
              $process_message .= "This Account could not be created - please check with the Administrator: $portal_email...<P>";
              $process_message .= "<a href=".$baseurl." target=_parent>Click here</a> to return...<P>";

              } # no Account created

           } else {# if contact created

           $process_message .= "This User could not be created - please check with the Administrator: $portal_email...<P>";
           $process_message .= "<a href=".$baseurl." target=_parent>Click here</a> to return...<P>";
         ...
         if ($body != NULL){
            ...
            if ($body != strip_tags($body)) {
               ...
               if ($source == 'Emails'){
                  ...
                  $body = $this->replacer("SCALASTICABR", "", $body);
